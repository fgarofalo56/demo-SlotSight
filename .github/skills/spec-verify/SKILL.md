---
name: spec-verify
description: Run every gate and check a spec's acceptance criteria one by one. Use when asked to "verify a spec", "check if this is done", "run the gates", "is spec NNN complete", or before marking any work finished. Produces a criterion-by-criterion table with evidence.
---

# Verify a spec

Verify `specs/NNN-name/spec.md` is genuinely satisfied.

> **Portable equivalent of the `/spec-verify` prompt file.** Prompt files work
...
Give a table: criterion → status → evidence.
